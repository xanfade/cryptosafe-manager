...
class ClipboardSettingsDialog(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        self._divider(content)
        self._section_title(content, "Уровень защиты")
        level_row = tk.Frame(content, bg=self.card)
        level_row.pack(fill="x", pady=(0, 12))
        for title, value in [("Базовый", "basic"), ("Продвинутый", "advanced"), ("Параноидальный", "paranoid")]:
            tk.Radiobutton(
                level_row,
                text=title,
                value=value,
        ...
        self._hint(content, "Базовый - мягкие проверки. Продвинутый - стандарт. Параноидальный - самый строгий режим.")

        self._divider(content)
        self._section_title(content, "Доверенные приложения")
        self._hint(content, "Каждое приложение с новой строки: chrome, Safari, Code, 1Password.exe")
